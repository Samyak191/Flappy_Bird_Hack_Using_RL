...
class Agent:
    ...
    def run(self, is_training=True, render=False):
        ...
        num_states= env.observation_space.shape[0] #inout dimensions
        num_actions= env.action_space.n #output dimenstions
        policy_dqn= DQN(num_states, num_actions).to(device)
        if is_training:
            memory=ReplayMemory(self.replay_memory_size)
            epsilon= self.epsilon_init
            target_dqn = DQN(num_states,num_actions).to(device)
            #copy weight and bias vals from policy to target
        ...
        for episode in itertools.count():
            ...
            while (not terminated and episode_rewards<self.reward_threshold):
                ...
                if terminated:
                    break
            print(f"for episode = {episode +1} total reward={episode_rewards} and epsilon ={epsilon}")
            ...
            #env.close() because we want to stop ma nnually
